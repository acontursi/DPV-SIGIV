...
/////////////////////
////  EVALUACIONES enero a agosto 2024 WFS

// - - > Variables para estilo
var ultimoclick = null;
var highlight_evaluaciones = {
/*  'fillColor': '',*/
...
var evaluaciones = 'http://10.7.20.214:8080/geoserver/sigiv/wfs?';
var defaultParameters = {
    service: 'WFS',
    version: '2.0.0',
    request: 'GetFeature',
    typeName: 'sigiv:ev_medicion',
    srsname: 'EPSG:4326',
...
fetch(evaluaciones + parameters)
    .then(function(response) {
        return response.json();
    })
    .then(function(data) {       // Se crea la capa GeoJSON a llamar con los datos obtenidos de la URL
      var fetch_evaluaciones = L.geoJson(data, {
        pointToLayer: function (feature, latlng) {
          return L.circleMarker(latlng, ev_estilo1)
        },
        onEachFeature: function (feature, layer) {
          /* EVENTO INTERNO que crea el HIGHLIGHT DE PUENTES (deja tildado al seleccionar)*/
          layer.on("click", function (e) {
              if (ultimoclick) {
                ultimoclick.setStyle(ev_estilo1) // Restablecer estilo de la capa puentes previamente seleccionada
              }
              layer.setStyle(highlight_evaluaciones);  // Establece el estilo de resaltado para la capa puentes clickeada
              ultimoclick = layer; // Actualiza la capa puentes seleccionada, vuelve al parámetro "layer"
          });
      }
      }).bindPopup(function (layer) {
        return (`<b> Fecha y hora: </b>` + layer.feature.properties.fecha_hora +  '<br>' +
        '<FONT COLOR="orange">' + '<b> Carril: </b>' + '</FONT>' + layer.feature.properties.carril + '<br>' +
        '<FONT COLOR="orange">' + '<b> Deflexiones (0, 20, 30, 60, 90): </b>' + '</FONT>' + layer.feature.properties.d0 + ' , ' + layer.feature.properties.d20 + ' , ' + layer.feature.properties.d30 + ' , ' + layer.feature.properties.d60 + ' , ' + layer.feature.properties.d90 + '<br>' +
        '<FONT COLOR="orange">' + '<b> Observaciones </b>' + '</FONT>' +layer.feature.properties.obs + '<br>'
      ...
      layerControl.addOverlay(fetch_evaluaciones, "Evaluaciones (enero a agosto - 2024)"); //---> se agrega la capa al árbol de capas
      ...
      visor.on('click', function() {
        if (ultimoclick) {
          ultimoclick.setStyle(ev_estilo1); // Restablecer el estilo de la capa seleccionada
